...
{
    // stores the tokens generated by the lexer
    // these are only compiled after replacing template values
    // this is so that functions are name-mangled correctly
...
struct BackendGenerator
{
    ...
    virtual std::string Mangle(const std::string& name, const std::vector<Type>& types = {}, bool isVar = false) = 0;
    virtual std::string Mangle(const std::string& name, const std::vector<Value>& types, bool isVar = false) = 0;
    ...
    std::string GenerateMangledFunction(const std::vector<std::pair<std::string, bool>>& namespaces, const std::string& function, const std::vector<Type>& values, bool isVar = false, bool isMember = false)
    {
        ...
        else
        {
            ...
            std::string result = Mangle(function, values, isVar);
            namespaceStack = namespaceStack__;
            return result;
        }
    }
    ...
    Type Reference(const Type& type)
    {
        Type result = Type::PointerID;
        result.next = new Type(type);
        return result;
    }
